...
import { TrendingUp, TrendingDown, X, Package, Hash } from 'lucide-react';
...
export const StockAdjustmentModal = ({ product, action, isOpen, onClose, onSubmit, isLoading }) => {
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
        ...
        try {
            ...
            const motivoFinal = values.motivo === 'otro' ? values.motivoPersonalizado : values.motivo;
            
            // Calcular la nueva cantidad de stock
            // const newStockQuantity = isIncrement 
            //     ? product.stockQuantity + parseInt(values.stockQuantity)
            //     : product.stockQuantity - parseInt(values.stockQuantity);

            let stockData = {
                hawaId: values.hawaId,
                quantity: values.stockQuantity,
                reason: motivoFinal,
                orderId: values.hawaId,
                adjustmentType: action,
                adjustmentAmount: parseInt(values.stockQuantity)
            };
                ...
                >
                    {({ values, isSubmitting }) => (
                        <Form className="p-6 space-y-6">
                            ...
                            {/* Preview del cambio */}
                            {values.stockQuantity && !isNaN(parseInt(values.stockQuantity)) && (
                            ...
                            {!isIncrement && values.stockQuantity && parseInt(values.stockQuantity) > product.stockQuantity && (
                            ...
                            {/* Validación adicional para motivo personalizado */}
                            {values.motivo === 'otro' && !values.motivoPersonalizado && (
                                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                    <p className="text-yellow-700 text-sm">
                                        ⚠️ Por favor especifique el motivo personalizado.
                                    </p>
                                </div>
                            )}
